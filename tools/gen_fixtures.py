...
def gen_logit_filters() -> dict:
    ...
    # ------------------------------------------------------------------
    # ...and the same cases under *this port's* semantics, which are a
    # deliberate hybrid: mlx_whisper's unmasked normalisation for the final
    # timestamp-vs-text rule, openai/whisper's value-based `>=` monotonicity
    # clause. Neither set above describes it, so neither can be asserted
    # against directly. `tools/reference_rules.py` carries the reasoning and
    # the implementation; it is numpy-only so that the expectations this
    # crate's tests actually assert against can be regenerated without a
    # working `mlx` install.
    # ------------------------------------------------------------------
    desc["timestamp_rules_port_semantics"] = port_semantics_cases(
        cases,
        logits_np,
        desc["row_labels"],
        sample_begin=sample_begin,
    ...
    desc["timestamp_rules_port_note"] = (
        "What src/decoding.rs implements: mlx_whisper's unmasked normalisation "
        "for the final timestamp-vs-text rule (DEC-1) combined with "
        "openai/whisper's value-based, >= monotonicity clause (DEC-2). "
        "Generated by tools/reference_rules.py. See tests/fixtures/README.md."
    )
    return desc
